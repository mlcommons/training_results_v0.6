...
class LowLevelRunner(object):
  ...
  def initialize(self,
                 ...
                 eval_model_fn,
                 params,
                 hparams,  # pylint: disable=unused-argument
                 config):
    ...
    # Build model.
    self.build_model(model_fn, eval_model_fn, params, config)
    ...
    for i in range(self.num_hosts):
      self.output_sess.run(self.dequeue_ops[i])
  ...
  def dequeue(self, decode_hparams):
    """Dequeue the prediction results."""
    queue = Queue.Queue()

    num_threads = min(8, self.num_hosts)
    def dequeue_thread_fn(sess, i):
      while True:
        item = queue.get(block=True)
        if item == _STOP:
          return
        hosts = [h for h in range(self.num_hosts) if h % num_threads == i]
        for host in hosts:
          self.predictions[host] = sess.run(self.dequeue_ops[host])
        queue.task_done()

    dequeue_threads = [None] * num_threads
    for i in range(num_threads):
      dequeue_threads[i] = threading.Thread(
          target=dequeue_thread_fn, args=(self.output_sess, i))
    ...
    for step in range(self.eval_steps):
      ...
      for j in range(self.num_hosts):
        for i in range(decode_hparams.batch_size // self.num_hosts):
          yield {
