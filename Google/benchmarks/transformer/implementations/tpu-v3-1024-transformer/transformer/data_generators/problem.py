...
GOOGLE_DATA_DIR = "/placer/prod/home/tensor2tensor/datasets/rs=6.3/v1"


def _normalize_data_dir(data_dir):
  if not data_dir:
    data_dir = GOOGLE_DATA_DIR
  # Add readahead for remote paths
  if data_dir.startswith("/cns") or data_dir.startswith("/placer"):
    data_dir = "/readahead/16M" + data_dir
  return data_dir
# END GOOGLE-INTERNAL
...
def cpu_count():
  ...
  cpu_info = system_info_lib.gather_cpu_info()
  if cpu_info.num_cores_allowed > 0:
    num_available_cores = cpu_info.num_cores_allowed
  else:
    num_available_cores = cpu_info.num_cores
  # END GOOGLE-INTERNAL
...
class Problem(object):
  ...
  def skip_random_fraction_when_training(self):
    ...
    return not FLAGS.deterministic_input
  ...
  def dataset(self,
    ...
    shuffle_files = (
        shuffle_files or
        shuffle_files is None and is_training and not FLAGS.deterministic_input)
  ...
  def _dataset_partition(self, mode, config, params):
    ...
    if mode != tf.estimator.ModeKeys.TRAIN or not hasattr(config, "tpu_config"):
      ...
      # Single host (sidecar eval).
      if config.tpu_config.num_shards < 16:
        return 0, 1
      # Distributed eval, assuming per host input for eval.
      else:
        eval_num_partitions = max(
            params["context"].num_hosts
            if "context" in params else params["tpu_num_hosts"], 1)
        if eval_num_partitions == 1:
          eval_partition_id = 0
        else:
          eval_partition_id = getattr(self, "_next_eval_partition_id", 0)
          self._next_eval_partition_id = eval_partition_id + 1
        tf.logging.info("eval_num_partitions = %d eval_partition_id = %d" %
                        (eval_num_partitions, eval_partition_id))
        assert eval_partition_id < eval_num_partitions
        return eval_partition_id, eval_num_partitions

    self._next_eval_partition_id = 0
  ...
  def input_fn(self,
    ...
    if fake_data:
      ...
      def _gen_fake_data(_):
        targets = tf.zeros([256, 1, 1], dtype=tf.int32)
        return (
            {
                "batch_prediction_key": tf.zeros([], dtype=tf.int32),
                "inputs": tf.zeros([256, 1, 1], dtype=tf.int32),
                "inputs_segmentation": tf.zeros([256], dtype=tf.int32),
                "inputs_position": tf.zeros([256], dtype=tf.int32),
                "targets": targets,
                "targets_segmentation": tf.zeros([256], dtype=tf.int32),
                "targets_position": tf.zeros([256], dtype=tf.int32),
            },
            targets)

      def _gen_fake_eval_data(_):
        return {
            "batch_prediction_key": tf.zeros([], dtype=tf.int32),
            "inputs": tf.zeros([97, 1, 1], dtype=tf.int32),
            "infer_targets": tf.zeros([97, 1, 1], dtype=tf.int32),
        }

      dataset = tf.data.Dataset.range(batch_size).repeat(16).map(
          _gen_fake_data if is_training else _gen_fake_eval_data)
    ...
    if not batch_size_means_tokens:
      ...
      if config and config.use_tpu:
        dataset = dataset.map(lambda data: (data, tpu_valid_size(data)),
                              num_parallel_calls=num_threads)
        dataset = dataset.filter(lambda data, pred: pred)
        dataset = dataset.map(lambda data, pred: data,
                              num_parallel_calls=num_threads)
        padded_shapes = self._pad_for_tpu(dataset.output_shapes, hparams)
        # on TPU, we use params["batch_size"], which specifies the number of
        # examples across all datashards
        batch_size = params["batch_size"]
        if hparams.pad_batch and not is_training:
          tf.logging.warn(
          ...
          dataset = dataset.map(
              ...
              num_parallel_calls=num_threads)
          # This is a hacky way to deal with decoding data that cannot be
          # divisiable by the global batch size.
          extra_padded_dataset = dataset.take(batch_size).map(
              empty_batch, num_parallel_calls=num_threads)
          extra_padded_dataset = extra_padded_dataset.repeat(16)
          dataset = dataset.concatenate(extra_padded_dataset)
        else:
          dataset = dataset.padded_batch(
    ...
    if (is_training and hasattr(hparams, "batch_shuffle_size") and
        hparams.batch_shuffle_size and not FLAGS.deterministic_input):
...
def empty_batch(features):
  emptied_features = {}
  for k, feature in features.items():
    emptied_features[k] = tf.zeros_like(feature, dtype=feature.dtype)
  return emptied_features
