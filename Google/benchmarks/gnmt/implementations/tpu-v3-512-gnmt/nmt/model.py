...
def input_projection(inputs, theta, max_seq_len):
  """Helper function to do projection on inputs."""
  max_time, batch_size, _ = inputs.shape
  factor = tf.maximum(1, 512 // batch_size)
  factored_batch = batch_size * factor
  factored_time = max_time // factor
  n = max_time * batch_size
  if n < 1024 or n % 512 != 0:
    return tf.reshape(
        tf.matmul(tf.reshape(inputs, [n, -1]), theta),
        [max_time, batch_size, -1])

  factored_shape = [factored_time, factored_batch, -1]
  input_reshape = tf.reshape(inputs, factored_shape)
  max_length = tf.where(
      tf.equal(max_seq_len % factor, 0), max_seq_len // factor,
      max_seq_len // factor + 1)

  @tf.custom_gradient
  def _input_projection(inputs, theta):
    """Function that does projection with early stop."""

    def cell_fn(theta, output, i):
      input_slice = tf.gather(input_reshape, i)
      output = inplace_ops.alias_inplace_update(output, i,
                                                tf.matmul(input_slice, theta))
    ...
    def grad(dy):
      dtheta, _, dinput, _ = tf.while_loop(
          lambda dtheta, dy, dinput, i: i < max_length, cell_grad_fn, [
              tf.zeros(theta.shape, dy.dtype), dy,
              tf.zeros_like(input_reshape),
              tf.zeros([], tf.int32)
          ])
      return dinput, dtheta

    output = tf.while_loop(lambda theta, output, i: i < max_length, cell_fn, [
        theta,
        tf.zeros(factored_shape[:2] + [theta.shape[-1]], inputs.dtype),
        tf.zeros([], tf.int32)
    ])[1]
    return output, grad

  return tf.reshape(
      _input_projection(input_reshape, theta), [max_time, batch_size, -1])
...
def build_uni_rnn(inputs,
  ...
  inp = {"rnn": input_projection(inputs["rnn"], input_kernel, max_seq_len)}
  if "padding" in inputs:
  ...
  def _build_decoder(self, encoder_outputs, hparams):
    ...
    with tf.variable_scope("decoder", reuse=tf.AUTO_REUSE) as decoder_scope:
      ...
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        ...
        inp = {"rnn": input_projection(o, input_kernels[0], max_seq_len)}
        ...
        for i in range(1, 4):
          ...
          inp = {"rnn": input_projection(c, input_kernels[i], max_seq_len)}
