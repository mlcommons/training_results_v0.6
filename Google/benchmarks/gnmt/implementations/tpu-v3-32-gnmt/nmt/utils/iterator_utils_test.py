...
class IteratorUtilsTest(tf.test.TestCase):
  ...
  def testGetIterator(self):
    ...
    hparams = tf.contrib.training.HParams(
        random_seed=3, num_buckets=1, eos="eos", sos="sos")
    batch_size = 2
    src_max_len = 5
  ...
  def testGetIteratorWithSkipCount(self):
    ...
    with self.test_session() as sess:
      ...
      self.assertAllEqual(
          [
              [4, -1, -1, 0, 3],  # "f" == unknown, "e" == unknown, a
              [4, 2, 2, 0, 3]
          ],  # c c a
          features["source"])
      self.assertAllEqual([5, 5], features["source_sequence_length"])
      self.assertAllEqual(
          [[4, 2, 2],    # sos c c
           [4, 0, 1]],   # sos a b
          features["target_input"])
  ...
  def testGetInferIterator(self):
    ...
    with self.test_session() as sess:
      ...
      self.assertAllEqual(
          ...
          features["source"])
      self.assertAllEqual([5, 4], features["source_sequence_length"])
