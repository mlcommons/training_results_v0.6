...
def resnet(units, num_stages, filter_list, num_classes, image_shape, batch_size, bottle_neck=True, workspace=256, dtype='float32', memonger=False,
    ...
    for i in range(num_stages):
        body, nchw_shape = res_unit(body, nchw_shape, filter_list[i+1], (1 if i==0 else 2, 1 if i==0 else 2), False,
        ...
        for j in range(units[i]-1):
            body, nchw_shape = res_unit(body, nchw_shape, filter_list[i+1], (1,1), True, name='stage%d_unit%d' % (i + 1, j + 2),
                                        bottle_neck=bottle_neck, workspace=workspace,
                                        memonger=memonger, conv_layout=conv_layout, batchnorm_layout=batchnorm_layout,
                                        verbose=verbose, cudnn_bn_off=cudnn_bn_off, bn_eps = bn_eps, bn_mom=bn_mom,
                                        conv_algo=conv_algo, fuse_bn_relu=fuse_bn_relu, fuse_bn_add_relu=fuse_bn_add_relu,
