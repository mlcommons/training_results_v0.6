...
class WarmupMultiStepLR(torch.optim.lr_scheduler._LRScheduler):
    ...
    def __init__(self, optimizer, iterations, warmup_steps=0,
        ...
        mlperf_print(key=mlperf_compliance.constants.OPT_LR_ALT_DECAY_FUNC,
                     value=True)
        mlperf_print(key=mlperf_compliance.constants.OPT_LR_ALT_WARMUP_FUNC,
                     value=True)

        mlperf_print(key=mlperf_compliance.constants.OPT_LR_DECAY_INTERVAL,
                     value=self.decay_interval)
        mlperf_print(key=mlperf_compliance.constants.OPT_LR_DECAY_FACTOR,
                     value=self.decay_factor)
        mlperf_print(key=mlperf_compliance.constants.OPT_LR_DECAY_STEPS,
                     value=self.decay_steps)
        mlperf_print(key=mlperf_compliance.constants.OPT_LR_REMAIN_STEPS,
