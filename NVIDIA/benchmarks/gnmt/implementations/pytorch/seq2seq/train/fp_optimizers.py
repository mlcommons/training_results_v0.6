    ...
    def step(self, loss, optimizer, scheduler, update=True):
        ...
        if update:
            ...
            if isinstance(optimizer, FusedAdam):
                ...
                norm = utils.fused_norm(self.fp16_params.grad.data) / scaling_factor
            else:
...
class AMPOptimizer:
    ...
    def __init__(self, model, grad_clip=None, loss_scale=8192,
        ...
        self.loss_scaler = apex.amp._amp_state.loss_scalers[0]
    ...
    def step(self, loss, optimizer, scheduler, update=True):
        ...
        if update:
            if scheduler is not None and not self.loss_scaler._has_overflow:
                scheduler.step()
            optimizer.step()
            for p in self.model.parameters():
                p.grad = None
