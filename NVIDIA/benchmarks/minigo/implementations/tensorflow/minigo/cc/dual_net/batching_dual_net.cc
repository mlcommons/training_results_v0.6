...
ModelBatcher::ModelBatcher(std::unique_ptr<DualNet> model_impl,
                           size_t buffering,
                           bool copy)
    : model_impl_(std::move(model_impl)), buffering_(buffering), copy_(copy) {}
...
float* ModelBatcher::GetInput() {
  return model_impl_->GetInput();
}

DualNet::Output* ModelBatcher::GetOutput() {
  return model_impl_->GetOutput();
}
...
void ModelBatcher::MaybeRunBatchesLocked() {
   if(copy_){
       ...
       if (!queue_.empty() && queue_.size() >= GetBatchSize()) {
           RunBatchNoCopy();
       }
   }
}
...
void ModelBatcher::RunBatchNoCopy() {
  auto batch_size = GetBatchSize();
  ...
  num_batches_ += 1;
  num_inferences_ += batch_size * 8; //hard code vitual_loss here, only for printing anyway
...
float* BatchingDualNet::GetInput() {
  return batcher_->GetInput();
}

DualNet::Output* BatchingDualNet::GetOutput() {
  return batcher_->GetOutput();
}
...
BatchingDualNetFactory::BatchingDualNetFactory(
    ...
    copy_ = factory_impl_->NeedCopy();
    printf(copy_ ? "Need Copy: true\n" : "Need Copy: false\n");
}
...
std::unique_ptr<DualNet> BatchingDualNetFactory::NewDualNet(
  ...
  if (it == batchers_.end()) {
    auto batcher = std::make_shared<internal::ModelBatcher>(
        factory_impl_->NewDualNet(model_path), GetBufferCount(), copy_);
