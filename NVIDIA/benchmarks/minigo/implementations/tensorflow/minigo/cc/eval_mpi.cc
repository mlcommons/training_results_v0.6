...
#ifdef MPIBUILD
    auto a_stats = model_a.GetWinStats();
    auto b_stats = model_b.GetWinStats();
    int wins_a = a_stats.white_wins.total() +  a_stats.black_wins.total();
    int wins_b = b_stats.white_wins.total() +  b_stats.black_wins.total();
    float winrate = wins_a / (float)(wins_a + wins_b);
    printf("wins_a %d, wins_b %d, winrate %f\n", wins_a, wins_b, winrate);

    MPI_Comm icomm;
    MPI_Comm_get_parent(&icomm);
    MPI_Gather(&winrate, 1, MPI_FLOAT, NULL, 0, MPI_FLOAT, 0, icomm);
#endif
...
int main(int argc, char* argv[]) {
  ...
  // use same GPU as rank
  std::string gpus = std::to_string(rank*2);
  setenv("CUDA_VISIBLE_DEVICES", gpus.c_str(), 1);
  printf("CUDA_VISIBLE_DEVICES=%s\n", getenv("CUDA_VISIBLE_DEVICES"));

  printf("in eval, rank %d, size %d, gpu %d\n", rank, size, rank*2);
  fflush(stdout);

  // Redirect output to /dev/null
  std::ofstream outbuf("/dev/null");
  std::streambuf* cerr0 = std::cerr.rdbuf();
  std::streambuf* cout0 = std::cout.rdbuf();
  std::cerr.rdbuf(outbuf.rdbuf());
