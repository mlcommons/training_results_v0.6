 ...
 private:
  ...
  struct ThreadOptions {
    void Init(int thread_id, Random* rnd) {
      ...
      player_options.thread_id = thread_id;
...
int main(int argc, char* argv[]) {
  ...
  // Set GPU device (hardcoded for DGX1)
  int gpu = rank / 2;
  setenv("CUDA_VISIBLE_DEVICES", std::to_string(gpu).c_str(), 1);
  printf("CUDA_VISIBLE_DEVICES=%s\n", getenv("CUDA_VISIBLE_DEVICES"));

  printf("in selfplay, rank %d, size %d, gpu %d\n", rank, size, gpu);
  fflush(stdout);
