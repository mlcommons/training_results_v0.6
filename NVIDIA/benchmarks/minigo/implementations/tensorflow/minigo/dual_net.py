...
flags.DEFINE_list('lr_boundaries', [400000, 600000],
                           'The number of steps at which the learning rate will decay')

flags.DEFINE_list('lr_rates', [0.01, 0.001, 0.0001],
...
flags.DEFINE_bool('use_mgpu_horovod', False, 'Whether to use Horovod based multi-gpu training.')
...
def model_fn_sess(features, labels, mode, params):
    """
    ...
    # hack to be able to manually read the values from the flags file
    # for logging in the rl_loop processs
    def convert_elems(l, dtype):
      result = []
      for e in l:
        if isinstance(e, str):
          result.append(dtype(e.replace('[', '').replace(']', '')))
        else:
          result.append(e)
      return result

    params['lr_rates'] = convert_elems(params['lr_rates'], float)
    params['lr_boundaries'] = convert_elems(params['lr_boundaries'], int)
...
def get_estimator():
    if FLAGS.use_tpu:
        ...
        if FLAGS.use_mgpu_horovod:
            return _get_nontpu_estimator_hvd()
        else:
            return _get_nontpu_estimator()


def _get_nontpu_estimator_hvd():
    ...
    session_config.gpu_options.force_gpu_compatible = True # Force pinned memory
    session_config.intra_op_parallelism_threads = 1 # Avoid pool of Eigen threads
    session_config.inter_op_parallelism_threads = 4 # max(2, 40//hvd.size()-2)
...
def freeze_graph(model_path, use_trt=False, trt_batch=2048):
    ...
    if use_trt:
        ...
        with tf.gfile.GFile(model_path + '.pb.og', 'wb') as f:
