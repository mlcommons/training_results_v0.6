...
from mlperf_log_utils import mlperf_print, mlperf_submission_log, barrier
...
def main(args):
    ...
    mlperf_print(key=mlperf_compliance.constants.INIT_START, log_all_ranks=True)
    ...
    mlperf_print(key=mlperf_compliance.constants.GLOBAL_BATCH_SIZE, value=args.max_tokens*args.distributed_world_size)
    mlperf_print(key=mlperf_compliance.constants.OPT_NAME, value=args.optimizer)
    assert(len(args.lr) == 1)
    mlperf_print(key=mlperf_compliance.constants.OPT_BASE_LR, value=args.lr[0] if len(args.lr) == 1 else args.lr)
    mlperf_print(key=mlperf_compliance.constants.OPT_LR_WARMUP_STEPS, value=args.warmup_updates)
    ...
    mlperf_print(key=mlperf_compliance.constants.OPT_ADAM_BETA_1, value=eval(args.adam_betas)[0])
    mlperf_print(key=mlperf_compliance.constants.OPT_ADAM_BETA_2, value=eval(args.adam_betas)[1])
    mlperf_print(key=mlperf_compliance.constants.OPT_ADAM_EPSILON, value=args.adam_eps)
    ...
    while lr >= args.min_lr and epoch_itr.epoch < max_epoch and trainer.get_num_updates() < max_update and current_bleu < tgt_bleu:
        ...
        mlperf_print(key=mlperf_compliance.constants.BLOCK_START,
                     ...
                     sync=True)
        mlperf_print(key=mlperf_compliance.constants.EPOCH_START, metadata={'epoch_num': first_epoch}, sync=True)
        start = time.time()
        ...
        start = time.time()
        mlperf_print(key=mlperf_compliance.constants.EPOCH_STOP, metadata={'epoch_num': first_epoch}, sync=True)
...
def score(args, trainer, task, epoch_itr, subset):
    ...
    mlperf_print(key=mlperf_compliance.constants.EVAL_START, metadata={'epoch_num': epoch_itr.epoch}, sync=True)
    begin = time.time()
    ...
    mlperf_print(key=mlperf_compliance.constants.EVAL_STOP, metadata={'epoch_num': epoch_itr.epoch}, sync=True)
