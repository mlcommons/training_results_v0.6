...
if [ -e ${BBOX_TAR_BALL} ]; then
  ...
  if [ $BASE_URL_CHANGE ]; then
    ...
    BOUNDING_BOX_ANNOTATIONS="${BASE_URL}/ILSVRC2012_bbox_train_v2.tar.gz"
    BBOX_TAR_BALL="${BBOX_DIR}/annotations.tar.gz"
    echo "Saving annotations to ${BBOX_TAR_BALL}"
...
VALIDATION_TARBALL="${IMAGENET_OUT_DIR}/ILSVRC2012_img_val.tar"
...
if [ -e ${VALIDATION_TARBALL} ]; then
  ...
  wget -nd -c "${BASE_URL}/ILSVRC2012_img_val.tar"
fi
echo "VALIDATION_TARBALL path is ${VALIDATION_TARBALL}"
...
if [ -e ${TRAIN_TARBALL} ]; then
  ...
  echo "Downloading ${TRAIN_TARBALL} to ${BASE_URL}/${TRAIN_TARBALL}."
