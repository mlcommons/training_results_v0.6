...
class SyntheticDataIter(DataIter):
    def __init__(self, num_classes, data_shape, max_iter, dtype, layout='NCHW'):
    ...
    def provide_data(self):
        return [mx.io.DataDesc('data', self.data.shape, self.dtype, self.layout)]
    @property
...
def get_rec_iter(args, kv=None):
    ...
    if 'benchmark' in args and args.benchmark:
        ...
        train = SyntheticDataIter(args.num_classes, data_shape,
                args.num_examples / args.batch_size, np.float32, args.input_layout)
        return (train, None)
    if kv:
