...
#if MXNET_USE_NVTX
  std::string NodeToString(const nnvm::Node &node);
#endif
...
#if MXNET_USE_NVTX
  // cached nvtx operator name (needs a longer lifecycle than routine that creates them)
  std::unordered_set<std::string> cached_nvtx_opr_names_;
#endif
  // verbose logging
