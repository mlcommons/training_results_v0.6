...
const std::unordered_map<std::string, NDArray> &
GraphExecutor::in_arg_map() const {
  return in_arg_map_;
}

const std::unordered_map<std::string, NDArray> &
GraphExecutor::arg_grad_map() const {
  return arg_grad_map_;
}

const std::unordered_map<std::string, NDArray> &
GraphExecutor::aux_state_map() const {
 ...
 */
nnvm::Graph
GraphExecutor::InitFullGraph(nnvm::Symbol symbol,
  ...
  auto need_mirror = [do_mirror](const nnvm::Node &node) -> int {
    ...
    if (type == "Dropout")
      return false;
    if (get_node_attr(node, "__force_mirroring__", false))
      return true;
    if (do_mirror == 0)
      return false;
    if (type == "Convolution")
    ...
    if (type == "Concat")
      return false;
    if (type == "SoftmaxOutput")
      return false;
    if (type == "BatchNorm")
      return false;
    if (type == "CuDNNBatchNorm")
      return false;
    return true;
  };
  ...
  nnvm::Graph g_grad = nnvm::pass::Gradient(
      g, symbol.outputs, xs, head_grad_entry_, AggregateGradient, need_mirror,
...
void GraphExecutor::InitDataEntryMemory(std::vector<NDArray> *shared_pool) {
  ...
  for (size_t i = 0; i < data_entry_.size(); ++i) {
    ...
    if (storage_type == kDefaultStorage) {
      ...
      data_entry_[i] =
          NDArray(storage_type, vshape[i], data_context[i], true, vdtype[i]);
    }
    if (log_verbose_) {
...
#if MXNET_USE_NVTX
    std::string opr_description = inode.source->op()->name + NodeToString(*inode.source);
    auto iter = cached_nvtx_opr_names_.insert(opr_description).first;
    op_nodes_[nid].opr_name = iter->c_str();
#else
...
#if MXNET_USE_NVTX
    opr_names += NodeToString(*inode.source) + ",";
#endif
  }
...
#if MXNET_USE_NVTX
  std::string  GraphExecutor::NodeToString(const nnvm::Node &node) {
    std::ostringstream oss;
    oss << "{name=" << node.attrs.name << ";";
    auto &idx = graph_.indexed_graph();
    // Add sizes for inputs
    auto num_inputs = node.inputs.size();
    for (uint32_t i = 0; i != num_inputs; ++i) {
      auto &input_entry = node.inputs[i];
      auto entry_id = idx.entry_id(input_entry);
      auto &ndarray = data_entry_[entry_id];
      oss << "in" << i << "=" << ndarray.shape() << ";";
    }
    // Add sizes for outputs
    auto num_outputs = node.num_outputs();
    auto node_id = idx.node_id(&node);
    for (uint32_t i = 0; i != num_outputs; ++i) {
      auto entry_id = idx.entry_id(node_id, i);
      auto &ndarray = data_entry_[entry_id];
      oss << "out" << i << "=" << ndarray.shape() << ";";
    }
    // Add Node parameter info
    const std::unordered_map<std::string, std::string> &dict = node.attrs.dict;
