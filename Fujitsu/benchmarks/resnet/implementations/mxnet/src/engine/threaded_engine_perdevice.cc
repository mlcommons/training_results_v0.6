 ...
 private:
  ...
  inline void GPUWorker(Context ctx,
    ...
    while (task_queue->Pop(&opr_block)) {
      ...
      auto end_ptr = std::strchr(nvtx_name, '{');
      auto name_prefix_len = end_ptr ? end_ptr - nvtx_name : strlen(nvtx_name);
      auto color = nvtx::nameToColor(nvtx_name, name_prefix_len);
      nvtx::gpuRangeStart(color, nvtx_name);
#endif
