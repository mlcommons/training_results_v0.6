...
LINALG_XPU_BATCH_GEMM(cpu, mshadow::half::half_t)
...
// Specialization of linalg_batch_gemm<gpu, DType> for DType=mshadow::half::half_t.
template<> inline
void linalg_batch_gemm<gpu, mshadow::half::half_t>(const Tensor<gpu, 3, mshadow::half::half_t>& A,
  ...
  auto cublas_math_mode = GetEnvAllowTensorCore() ? CUBLAS_TENSOR_OP_MATH
                                                  : CUBLAS_DEFAULT_MATH;
  auto cublas_algo = GetEnvAllowTensorCore() ? CUBLAS_GEMM_DFALT_TENSOR_OP
                                                  : CUBLAS_GEMM_DFALT;
  auto previous_math_mode = SetCublasMathMode(blas_handle, cublas_math_mode);
  ...
  check_gemm(A[0], B[0], C[0], alpha, beta, tA, tB);
  using namespace mshadow::cuda;
  int batchCount = A.size(0);
  auto half_datatype = CUDA_R_16F;
...
#else
  LOG(FATAL) << "FP16 gemm requires CUDA version >= 9.0 from NVIDIA container with cublas patch!";
...
/*!
 * \brief Performs function `f`, setting alpha and beta as appropriate for `req`.
 *
 * \param A the first operand of the function
 * \param B the second operand of the function
 ...
 */
#define LINALG_GEMM_DISPATCH(f, dim) \
template<typename xpu, typename DType> \
inline void f(const Tensor<xpu, dim, DType>& A, \
                        const Tensor<xpu, dim, DType>& B, \
                        const Tensor<xpu, dim, DType>& C, \
                        bool tA, bool tB, Stream<xpu> *s, \
                        mxnet::OpReqType req) { \
  using namespace mxnet; \
  switch (req) { \
    ...
    case kWriteTo: \
    case kWriteInplace: \
      f(A, B, C, DType(1.0), DType(0.0), tA, tB, s); \
      break; \
    case kAddTo: \
      f(A, B, C, DType(1.0), DType(1.0), tA, tB, s); \
      break; \
    default: \
      LOG(FATAL) << "not reached"; \
  } \
...
LINALG_GEMM_DISPATCH(linalg_gemm, 2)
LINALG_GEMM_DISPATCH(linalg_batch_gemm, 3)
