...
void ConvolutionCompute<gpu>(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    if (param.cudnn_off) {
      ...
      if (param.cudnn_algo_fwd_prec != -1)
        desired_forward_compute_type = param.cudnn_algo_fwd_prec;
      if (param.cudnn_algo_bwd_prec != -1)
        desired_backward_compute_type = param.cudnn_algo_bwd_prec;
