...
#if MXNET_USE_CUDNN == 1 && CUDNN_MAJOR >= 4
template<typename DType>
static NhwcBatchNormOp<DType> &GetNhwcOp(const BatchNormParam& param) {
#if DMLC_CXX11_THREAD_LOCAL
  static thread_local NhwcBatchNormOp<DType> op;
#else
  static MX_THREAD_LOCAL NhwcBatchNormOp<DType> op;
#endif
  op.Init(param);
  return op;
}
...
void BatchNormCompute<gpu>(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH_EX(dtype, DType, AccReal, {
    if (!param.cudnn_off && NhwcBatchNormOp<DType>::Supports(param, dtype, shape, ctx.run_ctx.ctx))
      GetNhwcOp<DType>(param).Forward(ctx, in_data, req, outputs, aux_states);
    else if (param.act_type.has_value())
      LOG(FATAL) << "Fused ReLU activation is only supported with float16 NHWC Batchnorm on Volta.";
    else if (!param.cudnn_off && CuDNNBatchNormOp<DType>::Supports(param, dtype, shape))
...
void BatchNormGradCompute<gpu>(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH_EX(dtype, DType, AccReal, {
    if (!param.cudnn_off && NhwcBatchNormOp<DType>::Supports(param, dtype, shape, ctx.run_ctx.ctx))
      GetNhwcOp<DType>(param).Backward(ctx, inputs, req, outputs);
    else if (param.act_type.has_value())
      LOG(FATAL) << "Fused ReLU activation is only supported with float16 NHWC Batchnorm on Volta.";
    else if (!param.cudnn_off && CuDNNBatchNormOp<DType>::Supports(param, dtype, shape))
