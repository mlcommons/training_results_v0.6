...
#include "./mkldnn/mkldnn_batch_norm_add_relu-inl.h"
#endif
...
#if MXNET_USE_MKLDNN == 1
static inline bool SupportMKLDNNBN(const NDArray &input, const BatchNormAddReluParam &param) {
  TShape shape = input.shape();
  return SupportMKLDNN(input) && shape.ndim() == 4
      && param.axis == mxnet::op::batchnormaddrelu::DEFAULT_AXIS
      && shape[param.axis] % 8 == 0
      && !mxnet::op::batchnormaddrelu::disable_mkl;
}

void BatchNormAddReluComputeExCPU(const nnvm::NodeAttrs &attrs,
                           const OpContext &ctx,
  ...
  const BatchNormAddReluParam &param = nnvm::get<BatchNormAddReluParam>(attrs.parsed);
  // MKLDNN batchnormaddrelu only works well on the special MKLDNN layout.
  if (SupportMKLDNNBN(inputs[0], param) && inputs[0].IsMKLDNNData()) {
    std::vector<NDArray> in_data(inputs.begin(), inputs.begin() + batchnormaddrelu::kInMovingMean);
    std::vector<NDArray> aux_states(inputs.begin() + batchnormaddrelu::kInMovingMean, inputs.end());

    if (inputs[0].dtype() == mshadow::kFloat32) {
      MKLDNN_OPCHECK_INIT(false, outputs.size(), inputs, outputs);
      MKLDNNBatchNormAddReluForward<float>(ctx, param, in_data, req, outputs, aux_states);
      MKLDNN_OPCHECK_RUN(BatchNormAddReluCompute<cpu>, attrs, ctx, inputs, req, outputs);
      return;
    }
  }
  FallBackCompute(BatchNormAddReluCompute<cpu>, attrs, ctx, inputs, req, outputs);
}

void BatchNormAddReluGradComputeExCPU(const nnvm::NodeAttrs &attrs,
                               const OpContext &ctx,
                               const std::vector<NDArray> &inputs,
                               const std::vector<OpReqType> &req,
                               const std::vector<NDArray> &outputs) {
  ...
  const BatchNormAddReluParam &param = nnvm::get<BatchNormAddReluParam>(attrs.parsed);
  LOG(FATAL) << "I need to remove this code";
  ...
  if (SupportMKLDNNBN(inputs[0], param)
    ...
    in_data[batchnormaddrelu::kData] = inputs[3];
    in_data[batchnormaddrelu::kGamma] = inputs[4];
    in_data[batchnormaddrelu::kBeta] = inputs[5];
    aux_states[batchnormaddrelu::kMovingMean] = inputs[6];
    aux_states[batchnormaddrelu::kMovingVar] = inputs[7];
    const std::vector<NDArray> &in_grad = outputs;

    if (inputs[0].dtype() == mshadow::kFloat32) {
      MKLDNN_OPCHECK_INIT(true, outputs.size(), inputs, outputs);
      MKLDNNBatchNormAddReluBackward<float>(ctx, param, out_grad, in_data,
                                     out_data, req, in_grad, aux_states);
      MKLDNN_OPCHECK_RUN(BatchNormAddReluGradCompute<cpu>, attrs, ctx, inputs, req, outputs);
      return;
    }
  }
  FallBackCompute(BatchNormAddReluGradCompute<cpu>, attrs, ctx, inputs, req, outputs);
}
...
.set_attr<nnvm::FInferShape>("FInferShape", BatchNormAddReluShape)
.set_attr<nnvm::FInferType>("FInferType", BatchNormAddReluType)
.set_attr<FCompute>("FCompute<cpu>", BatchNormAddReluCompute<cpu>)
#if MXNET_USE_MKLDNN == 1
.set_attr<FComputeEx>("FComputeEx<cpu>", BatchNormAddReluComputeExCPU)
#endif
...
#if MXNET_USE_MKLDNN == 1
.set_attr<bool>("TIsMKLDNN", true)
.set_attr<FComputeEx>("FComputeEx<cpu>", BatchNormAddReluGradComputeExCPU)
#endif
