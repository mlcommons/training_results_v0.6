...
class NhwcBatchNormAddReluOp {
 public:
  ...
  void Forward(const OpContext &ctx,
    ...
    Init(in_data[nhwcbatchnormaddrelu::kData], s);
 ...
 */
  static bool Supports(const BatchNormAddReluParam &param, int dtype, const TShape& shape,
                       const Context& ctx) {
 ...
 private:
  void Init(const TBlob &in_data, mshadow::Stream<gpu> *s) {
      ...
      nhwc_layer_->setInputDescriptor(tensor_format, data_type, n, c, h, w);
