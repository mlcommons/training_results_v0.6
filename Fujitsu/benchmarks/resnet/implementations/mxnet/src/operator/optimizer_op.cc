...
#ifdef LARGE_BATCH
DMLC_REGISTER_PARAMETER(MultiSGDMomParamEx);
DMLC_REGISTER_PARAMETER(SGDMomParamEx);
#endif
...
#ifdef LARGE_BATCH
NNVM_REGISTER_OP(multi_sgd_two_stage_lars_mom_update)
.describe(R"code(Momentum update function for Stochastic Gradient Descent (SGD) optimizer.

)code" ADD_FILELINE)
.set_num_inputs([](const nnvm::NodeAttrs& attrs) {
    const MultiSGDMomParamEx& param = dmlc::get<MultiSGDMomParamEx>(attrs.parsed);
...
.set_attr_parser(ParamParser<MultiSGDMomParamEx>)
.set_attr<nnvm::FInferShape>("FInferShape", MultiSGDShape<MultiSGDMomParamEx, 4>)
.set_attr<nnvm::FInferType>("FInferType", ElemwiseType<-1, -1>)
.set_attr<nnvm::FListInputNames>("FListInputNames",
  [](const NodeAttrs& attrs) {
    uint32_t num_args = dmlc::get<MultiSGDMomParamEx>(attrs.parsed).num_weights;
    std::vector<std::string> ret;
    for (uint32_t i = 0; i < num_args; ++i) {
      ret.push_back(std::string("weight_") + std::to_string(i));
      ret.push_back(std::string("grad_") + std::to_string(i));
      ret.push_back(std::string("tmp_") + std::to_string(i));
      ret.push_back(std::string("mom_") + std::to_string(i));
    }
    return ret;
  })
...
.set_attr<FCompute>("FCompute<cpu>", MultiSGDTwoStageLarsMomUpdate<cpu, type_identity, 4>)
.add_argument("data", "NDArray-or-Symbol[]", "Weights, gradients and momentum")
.add_arguments(MultiSGDMomParamEx::__FIELDS__());


NNVM_REGISTER_OP(multi_sgd_larc_mom_update)
.describe(R"code(Momentum update function for Stochastic Gradient Descent (SGD) optimizer.
...
.set_attr_parser(ParamParser<MultiSGDMomParamEx>)
.set_attr<nnvm::FInferShape>("FInferShape", MultiSGDShape<MultiSGDMomParamEx, 3>)
.set_attr<nnvm::FInferType>("FInferType", ElemwiseType<-1, -1>)
.set_attr<nnvm::FListInputNames>("FListInputNames",
  [](const NodeAttrs& attrs) {
    uint32_t num_args = dmlc::get<MultiSGDMomParamEx>(attrs.parsed).num_weights;
...
.set_attr<FCompute>("FCompute<cpu>", MultiSGDLarcMomUpdate<cpu, type_identity, 3>)
.add_argument("data", "NDArray-or-Symbol[]", "Weights, gradients and momentum")
.add_arguments(MultiSGDMomParamEx::__FIELDS__());
...
NNVM_REGISTER_OP(multi_sgd_mom_update_ex)
.describe(R"code(Momentum update function for Stochastic Gradient Descent (SGD) optimizer.
...
.set_attr<FCompute>("FCompute<cpu>", MultiSGDMomUpdateEx<cpu, type_identity, 4>)
.add_argument("data", "NDArray-or-Symbol[]", "Weights, gradients and momentum")
.add_arguments(MultiSGDMomParamEx::__FIELDS__());
...
#ifdef LARGE_BATCH
NNVM_REGISTER_OP(multi_mp_sgd_two_stage_lars_mom_update)
.describe(R"code(Momentum update function for multi-precision Stochastic Gradient Descent (SGD) optimizer.
)code" ADD_FILELINE)
.set_num_inputs([](const nnvm::NodeAttrs& attrs) {
    const MultiSGDMomParamEx& param = dmlc::get<MultiSGDMomParamEx>(attrs.parsed);
    return static_cast<uint32_t>(param.num_weights * 5);
  })
.set_num_outputs([](const nnvm::NodeAttrs& attrs) {
    const MultiSGDMomParamEx& param = dmlc::get<MultiSGDMomParamEx>(attrs.parsed);
    return static_cast<uint32_t>(param.num_weights);
...
.set_attr_parser(ParamParser<MultiSGDMomParamEx>)
.set_attr<nnvm::FInferShape>("FInferShape", MultiSGDShape<MultiSGDMomParamEx, 5>)
.set_attr<nnvm::FInferType>("FInferType", MP_MultiSGD_InferType<MultiSGDMomParamEx, 5, 3>)
.set_attr<nnvm::FListInputNames>("FListInputNames",
  [](const NodeAttrs& attrs) {
    uint32_t num_args = dmlc::get<MultiSGDMomParamEx>(attrs.parsed).num_weights;
    std::vector<std::string> ret;
...
.set_attr<FCompute>("FCompute<cpu>", MultiSGDTwoStageLarsMomUpdate<cpu, single_precision, 5>)
.add_argument("data", "NDArray-or-Symbol[]", "Weights")
.add_arguments(MultiSGDMomParamEx::__FIELDS__());



NNVM_REGISTER_OP(multi_mp_sgd_larc_mom_update)
.describe(R"code(Momentum update function for multi-precision Stochastic Gradient Descent (SGD) optimizer.
)code" ADD_FILELINE)
.set_num_inputs([](const nnvm::NodeAttrs& attrs) {
    const MultiSGDMomParamEx& param = dmlc::get<MultiSGDMomParamEx>(attrs.parsed);
...
.set_attr_parser(ParamParser<MultiSGDMomParamEx>)
.set_attr<nnvm::FInferShape>("FInferShape", MultiSGDShape<MultiSGDMomParamEx, 4>)
.set_attr<nnvm::FInferType>("FInferType", MP_MultiSGD_InferType<MultiSGDMomParamEx, 4, 2>)
.set_attr<nnvm::FListInputNames>("FListInputNames",
  [](const NodeAttrs& attrs) {
    uint32_t num_args = dmlc::get<MultiSGDMomParamEx>(attrs.parsed).num_weights;
    std::vector<std::string> ret;
...
.set_attr<FCompute>("FCompute<cpu>", MultiSGDLarcMomUpdate<cpu, single_precision, 4>)
.add_argument("data", "NDArray-or-Symbol[]", "Weights")
.add_arguments(MultiSGDMomParamEx::__FIELDS__());


NNVM_REGISTER_OP(multi_mp_sgd_mom_update_ex)
.describe(R"code(Momentum update function for multi-precision Stochastic Gradient Descent (SGD) optimizer.
...
.set_attr<FCompute>("FCompute<cpu>", MultiSGDMomUpdateEx<cpu, single_precision, 5>)
.add_argument("data", "NDArray-or-Symbol[]", "Weights")
.add_arguments(MultiSGDMomParamEx::__FIELDS__());
#endif
