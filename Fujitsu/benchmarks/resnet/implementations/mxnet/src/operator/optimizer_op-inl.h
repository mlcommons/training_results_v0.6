...
#ifdef LARGE_BATCH
struct MultiSGDMomParamEx : public dmlc::Parameter<MultiSGDMomParamEx> {
  nnvm::Tuple<float> lrs;
  nnvm::Tuple<float> wds;
  float momentum;
  float rescale_grad;
  ...
  int num_weights;
  float eta; // addition
  int world_size; // addition
  unsigned long long int lars_flags; // addition
  DMLC_DECLARE_PARAMETER(MultiSGDMomParamEx) {
    DMLC_DECLARE_FIELD(lrs)
    .describe("Learning rates.");
    DMLC_DECLARE_FIELD(wds)
    .describe("Weight decay augments the objective function with a "
    ...
    .set_default(1)
    .describe("Number of updated weights.");
    DMLC_DECLARE_FIELD(tmp_coeff)
    .set_default(1.0f)
    .describe("Temporaly coefficient value.");
    DMLC_DECLARE_FIELD(best_coeff)
    .set_default(1.0f)
    .describe("Best coefficient value .");
    DMLC_DECLARE_FIELD(tmp_wd_coeff)
    .set_default(1.0f)
    .describe("Temporaly coefficient value.");
    DMLC_DECLARE_FIELD(best_wd_coeff)
    .set_default(1.0f)
    .describe("Best coefficient value .");
    DMLC_DECLARE_FIELD(eta)
    .set_default(1.0f)
    .describe("eta value.");
    DMLC_DECLARE_FIELD(world_size)
    .set_default(1)
    .describe("num of mpi ranks .");
    DMLC_DECLARE_FIELD(lars_flags)
    .set_default(0)
    .describe("LARS flags for each layer");
  }
};
#endif
  ...
  MSHADOW_XINLINE static void Map(int i, const MultiSGDKernelParam<DType, MPDType>& param,
    ...
    for (int index = 0; index < param.count; ++index) {
      if ((size_t)i < param.sizes[index]) {
        //MPDType w = has_mixed_precision ? param.weights32[index][i] :
        //                                  MPDType(param.weights[index][i]);
        MPDType w = param.tmp_w[index][i];
        MPDType mom = has_momentum ? param.mom[index][i] : MPDType(0);
        if (param.clip_gradient >= 0.0f) {
          mom = param.momentum*mom
                - param.wds[index]*w*param.best_wd_coeff
                - param.lrs[index]*param.best_coeff
                *mshadow_op::clip::Map(param.rescale_grad *
                                       static_cast<MPDType>(param.grads[index][i]),
                                     param.clip_gradient);
        } else {
          mom = param.momentum*mom
                - param.wds[index]*w*param.best_wd_coeff
                - param.lrs[index]*param.best_coeff*param.rescale_grad*static_cast<MPDType>(param.grads[index][i]);
        }
        if (has_momentum) {
          param.mom[index][i] = mom;
        }
...
template<typename xpu,
         ...
         typename MPDType,
         int input_stride = 3>
MultiSGDKernelParam<DType, MPDType> FillMultiFJSGDMomKernelParam(const nnvm::NodeAttrs& attrs,
                                                            const OpContext &ctx,
                                                            const std::vector<TBlob> &inputs,
                                                            const std::vector<TBlob> &outputs) {
  using namespace mxnet_op;
  const MultiSGDMomParamEx& p = nnvm::get<MultiSGDMomParamEx>(attrs.parsed);
  Stream<xpu>* s = ctx.get_stream<xpu>();
  MultiSGDKernelParam<DType, MPDType> param =
    FillMultiSGDKernelParam<xpu,
                            DType,
                            ...
                            input_stride>(attrs, ctx, inputs, outputs);
  param.momentum = p.momentum;
  param.tmp_coeff = p.tmp_coeff;
  param.best_coeff = p.best_coeff;
  param.tmp_wd_coeff = p.tmp_wd_coeff;
  param.best_wd_coeff = p.best_wd_coeff;
  for (int i = 0; i < param.count; ++i) {
    param.tmp_w[i] = inputs[i * input_stride + 2].FlatTo2D<xpu, MPDType>(s).dptr_;
    param.mom[i] = inputs[i * input_stride + 3].FlatTo2D<xpu, MPDType>(s).dptr_;
  }
...
template<typename xpu,
         ...
         typename MPDType,
         int input_stride = 3>
MultiSGDKernelParam<DType, MPDType> FillMultiSGDMomKernelParamEx(const nnvm::NodeAttrs& attrs,
                                                            const OpContext &ctx,
                                                            const std::vector<TBlob> &inputs,
                                                            const std::vector<TBlob> &outputs) {
  using namespace mxnet_op;
...
template<typename xpu, template<typename> class MPTypeChooser, int input_stride>
inline void MultiSGDTwoStageLarsMomUpdate(const nnvm::NodeAttrs& attrs,
                                const OpContext &ctx,
                                const std::vector<TBlob> &inputs,
                                const std::vector<OpReqType> &req,
                                const std::vector<TBlob> &outputs) {
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    using MPDType = typename MPTypeChooser<DType>::type;
    MultiSGDKernelParam<DType, MPDType> param =
     FillMultiFJSGDMomKernelParam<xpu,
                                     DType,
                                     MPDType,
                                     input_stride>(attrs, ctx, inputs, outputs);
    param.eta = p.eta;
    param.world_size = p.world_size;
    param.lars_flags = p.lars_flags;

    param.has_momentum = 1;
    param.has_mixed_precision = !std::is_same<DType, MPDType>::value;

    if( std::is_same<DType, MPDType>::value ){
      Kernel4Larc<MultiSGDKernel<MPDType, 
        true, 
        !std::is_same<DType, MPDType>::value>, MPDType, DType,
        xpu>::LarsLaunch(s, param.count, param, req[0]);
      ...
      Kernel4Larc<MultiSGDKernel<MPDType, 
        true, 
        !std::is_same<DType, MPDType>::value>, MPDType, DType,
        xpu>::TwoStageLarsLaunch(s, param.count, param, req[0]);
    }
  });
}
...
template<typename xpu, template<typename> class MPTypeChooser, int input_stride>
inline void MultiSGDLarcMomUpdate(const nnvm::NodeAttrs& attrs,
                                const OpContext &ctx,
                                const std::vector<TBlob> &inputs,
                                const std::vector<OpReqType> &req,
                                const std::vector<TBlob> &outputs) {
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    using MPDType = typename MPTypeChooser<DType>::type;
    MultiSGDKernelParam<DType, MPDType> param =
     FillMultiSGDMomKernelParamEx<xpu,
                                     DType,
                                     MPDType,
                                     input_stride>(attrs, ctx, inputs, outputs);
    param.eta = p.eta;
    ...
    Kernel4Larc<MultiSGDKernel<MPDType, 
        true, 
        !std::is_same<DType, MPDType>::value>, MPDType, DType,
        xpu>::LarcLaunch(s, param.count, param, req[0]);
...
template<typename xpu, template<typename> class MPTypeChooser, int input_stride>
inline void MultiSGDMomUpdateEx(const nnvm::NodeAttrs& attrs,
                                const OpContext &ctx,
                                const std::vector<TBlob> &inputs,
                                const std::vector<OpReqType> &req,
                                const std::vector<TBlob> &outputs) {
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    ...
    param.best_wd_coeff = p.best_wd_coeff;
    param.tmp_wd_coeff = p.tmp_wd_coeff;
    Kernel<MultiSGDKernelEx<MPDType,
                             true,
                             !std::is_same<DType, MPDType>::value>,
                             xpu>::Launch(s, param.max_size, param, req[0]);
  });
...
#ifdef FJ_LBSGD
struct SGDMomParamEx : public dmlc::Parameter<SGDMomParamEx> {
  float lr;
  float momentum;
  float wd;
  float rescale_grad;
  ...
  float best_coeff;
  float best_wd_coeff;
  DMLC_DECLARE_PARAMETER(SGDMomParamEx) {
    DMLC_DECLARE_FIELD(lr)
    .describe("Learning rate");
    DMLC_DECLARE_FIELD(momentum)
    .set_default(0.0f)
