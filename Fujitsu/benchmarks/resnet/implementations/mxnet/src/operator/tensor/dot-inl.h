...
void DotForward_(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    if (inputs[0].ndim() == 1 && inputs[1].ndim() == 1) {
      ...
      linalg_gemm(input0, input1, out, param.transpose_a, param.transpose_b, s, req[0]);
    }
  });
}
...
void DotBackward_(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    if (inputs[1].ndim() == 1 && inputs[2].ndim() == 1) {
      ...
      // The 4 cases to consider, and their implementation:
      ...
      if (!param.transpose_b) {
        linalg_gemm(mlhs_data, mout_grad, mrhs_grad, !param.transpose_a, false, s, req[1]);
      } else {
        // To produce the transpose of the above result, we swap the order of the input data and
        // transpose flags, and also complement the flag sense per the rule (A*B)^T = B^T * A^T.
        linalg_gemm(mout_grad, mlhs_data, mrhs_grad, true, param.transpose_a, s, req[1]);
      }

      if (!param.transpose_a) {
        linalg_gemm(mout_grad, mrhs_data, mlhs_grad, false, !param.transpose_b, s, req[0]);
      } else {
        // To produce the transpose of the above result, we swap the order of the input data and
        // transpose flags, and also complement the flag sense per the rule (A*B)^T = B^T * A^T.
        linalg_gemm(mrhs_data, mout_grad, mlhs_grad, param.transpose_b, true, s, req[0]);
      }
    }
  });
}
...
void BatchDotForward_(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    ...
    linalg_batch_gemm(mlhs, mrhs, out, param.transpose_a, param.transpose_b, s, req[0]);
  });
}
...
void BatchDotBackward_(const nnvm::NodeAttrs& attrs,
  ...
  MSHADOW_REAL_TYPE_SWITCH(outputs[0].type_flag_, DType, {
    ...
    if (!param.transpose_b) {
      linalg_batch_gemm(mlhs_data, mout_grad, mrhs_grad, !param.transpose_a, false, s, req[1]);
    } else {
      // To produce the transpose of the above result, we swap the order of the input data and
      // transpose flags, and also complement the flag sense per the rule (A*B)^T = B^T * A^T.
      linalg_batch_gemm(mout_grad, mlhs_data, mrhs_grad, true, param.transpose_a, s, req[1]);
    }

    if (!param.transpose_a) {
      linalg_batch_gemm(mout_grad, mrhs_data, mlhs_grad, false, !param.transpose_b, s, req[0]);
    } else {
      // To produce the transpose of the above result, we swap the order of the input data and
      // transpose flags, and also complement the flag sense per the rule (A*B)^T = B^T * A^T.
      linalg_batch_gemm(mrhs_data, mout_grad, mlhs_grad, param.transpose_b, true, s, req[0]);
    }
  });
}
