...
def check_consistency(sym, ctx_list, scale=1.0, grad_req='write',
                      ...
                      use_uniform=False, rand_type=None):
 ...
 {'ctx': mx.cpu(0), 'concat_arg1': (2, 10), 'concat_arg0': (2, 10),\
  'type_dict': {'concat_arg0': np.float32, 'concat_arg1': np.float32}}]
    ...
    def smaller_dtype(dt1, dt2):
        return dt1 if np.dtype(dt1).itemsize < np.dtype(dt2).itemsize else dt2
    ...
    if rand_type is None:
        for n, arr in exe_list[0].arg_dict.items():
            if n not in arg_params:
                arg_param_dtypes[n] = arr.dtype
        for exe in exe_list:
            for n, arr in exe.arg_dict.items():
                if n not in arg_params:
                    arg_param_dtypes[n] = smaller_dtype(arr.dtype, arg_param_dtypes[n])
    ...
    for i, exe in enumerate(exe_list):
        ...
        for name, arr in zip(output_names, exe.outputs):
            ...
            gtarr = gt[name].astype(arr.dtype).asnumpy()
            try:
    ...
    # train
    if grad_req != 'null':
        # Perform forward() first
        for exe in exe_list:
            exe.forward(is_train=True)
        # This routine historically has taken the forward() output data of each exe and
        # made that the out_grad data for backward().  This is a little wierd, but easily
        # programmed.  The problem with this is that variations in the forward output
        # among exe's are compounded by the backward pass.  To correct this we take
        # the data from exe_list[0], and we cast it to the least precise dtype across
        # all exe's (for each output).  This way, the backward pass is fed identical
        # data across all exe's.
        ...
        # We use the first executor's output data as the gradient data to pass to all
        # executor's backward() call.
