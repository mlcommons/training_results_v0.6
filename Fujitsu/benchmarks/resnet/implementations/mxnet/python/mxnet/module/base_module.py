...
class BaseModule(object):
    ...
    - computation
        ...
        - `update_metric(metric, labels, pre_sliced=False, label_pads=None)`: update performance
          metric for the previous forward computed results.
        ...
        for nbatch, eval_batch in enumerate(eval_data):
            ...
            if isinstance(eval_batch, list):
                self.update_metric(eval_metric, [eb.label for eb in eval_batch],
                                   pre_sliced=True, label_pads=[eb.pad for eb in eval_batch])
            else:
                self.update_metric(eval_metric, eval_batch.label,
                                   pre_sliced=False, label_pads=eval_batch.pad)
            ...
            while not end_of_batch:
                ...
                if isinstance(data_batch, list):
                    self.update_metric(eval_metric,
                                       ...
                                       pre_sliced=True,
                                       label_pads=[db.pad for db in data_batch])
                else:
                ...
                if batch_end_callback is not None:
                    if 'lr_scheduler' in optimizer_params.keys():
                        lr = optimizer_params['lr_scheduler'](num_update) # pylint: disable=unused-variable
