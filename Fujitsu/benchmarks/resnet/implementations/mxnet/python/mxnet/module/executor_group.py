...
class DataParallelExecutorGroup(object):
    ...
    def update_metric(self, eval_metric, labels, pre_sliced, label_pads):
       ...
       """
        if label_pads is None:
            label_pads = [0,] * len(labels) if pre_sliced else 0

        for current_exec, (texec, islice) in enumerate(zip(self.execs, self.slices)):
            if not pre_sliced:
                trimmed_start = min(islice.start, self.batch_size - label_pads)
                trimmed_stop = min(islice.stop, self.batch_size - label_pads)
                trimmed_size = trimmed_stop - trimmed_start
                if trimmed_size <= 0:
                    break
                trimmed_slice = slice(trimmed_start, trimmed_stop)
                labels_slice = []
                for label, axis in zip(labels, self.label_layouts):
                    if axis == 0:
                        # slicing NDArray along axis 0 can avoid copying
                        labels_slice.append(label[trimmed_slice])
                    elif axis > 0:
                        # pylint: disable=no-member
                        label_my_slice = \
                            nd.slice_axis(label, axis=axis, begin=trimmed_slice.start,
                                          end=trimmed_slice.stop).as_in_context(label.context)
                        # pylint: enable=no-member
                        labels_slice.append(label_my_slice)
                    else:
                        labels_slice.append(label)
            else:
                pad = label_pads[current_exec] if label_pads[current_exec] is not None else 0
                if pad == 0:
                    labels_slice = labels[current_exec]
                else:
                    labels_slice = [label[0:label.shape[0]-pad] for label in labels[current_exec]]
                trimmed_size = islice.stop - islice.start - pad
                if trimmed_size <= 0:
                    break

            labels_ = OrderedDict(zip(self.label_names, labels_slice))
            islice_batch_size = islice.stop - islice.start
            outputs_slice = []
            for out in texec.outputs:
                # RNN's may have a flattened first dimension size = batch_size * sentence_length.
                # We assume pad size refers to the number of trimmed samples.
                assert out.shape[0] % islice_batch_size == 0,\
                    "output length {} not a multiple of slice batch_size {}".format(
                        out.shape[0], islice_batch_size)
                values_per_sample = out.shape[0] // islice_batch_size
                outputs_slice.append(out[0:trimmed_size*values_per_sample])
