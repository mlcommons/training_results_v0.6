...
from ..ndarray import (NDArray, zeros, clip, sqrt, cast, maximum, abs as NDabs, array, multiply, norm)
from ..ndarray import (sgd_update, sgd_mom_update, adam_update, rmsprop_update, rmspropalex_update,
                       mp_sgd_update, mp_sgd_mom_update, square, ftrl_update, ftml_update,
                       signsgd_update, signum_update,
                       multi_sgd_update, multi_sgd_mom_update, multi_mp_sgd_update,
                       multi_sgd_mom_update_ex,
                       multi_mp_sgd_mom_update_ex,
                       multi_sgd_two_stage_lars_mom_update, multi_mp_sgd_two_stage_lars_mom_update,
                       multi_sgd_larc_mom_update, multi_mp_sgd_larc_mom_update,
    ...
    **lazy updates** are applied by::
    ...
    multi_precision: bool, optional
        Flag to control the internal precision of the optimizer.::
            ...
            Turning this on can improve convergence and accuracy when training with float16.
    """
    def __init__(self, momentum=0.0, lazy_update=True, bias_wd=False, bn_lr_decay=True, **kwargs):
        self.bias_wd = bias_wd
        self.bn_lr_decay = bn_lr_decay
        super(SGD, self).__init__(**kwargs)
        self.momentum = momentum
        self.lazy_update = lazy_update
        self.aggregate_num = int(os.getenv('MXNET_OPTIMIZER_AGGREGATION_SIZE', 4))
        self.whole_layer_num = int(os.getenv('MXNET_WHOLE_LAYER_NUM', 160))
        self.lars_active = int(os.getenv('USE_FJSGD_LARS_ACTIVE',0))
        self.larc_scale_active = int(os.getenv('USE_FJSGD_LARC_SCALE_ACTIVE',0))
        self.larc_clip_active = int(os.getenv('USE_FJSGD_LARC_CLIP_ACTIVE',0))
        self.larc_eta = float(os.getenv('USE_FJSGD_LARC_ETA',0.1))
        self.world_size = int(os.getenv('OMPI_COMM_WORLD_SIZE',1))
        self.zero_bn_decay = int(os.getenv('USE_ZERO_BN_DECAY',0))
        self.tmp_coeff = 1.0
        self.tmp_wd_coeff = 1.0
        self.best_coeff = 1.0
        self.best_wd_coeff = 1.0
        self.print_values = int(os.getenv('PRINT_FJSGD_VALUES',0)) != 0 and int(os.getenv('OMPI_COMM_WORLD_RANK', 0)) == 0

    def set_wd_mult(self, args_wd_mult):
        self.wd_mult = {}
        for n in self.idx2name.values():
            if not (n.endswith('_weight') or (self.bias_wd and n.endswith('_bias'))):
                self.wd_mult[n] = 0.0
        if self.sym_info:
            attr, arg_names = self.sym_info
            for name in arg_names:
    ...
    def create_norms(self, index, weight):
        momentum = None
        weight_master_copy = weight.astype(numpy.float32)
        return self.create_state(index, weight_master_copy)

    def _update_impl(self, indices, weights, grads, tmp_w, states, multi_precision=False):
        aggregate = True
        lars_flags = 0
        ...
        if self.lars_active != 0:   
            if self.print_values and 0 in indices:
                logging.info('lr(base,coeff,actual) {:.10e} {:.10e} {:.10e}'.format(lrs[0], self.best_coeff, lrs[0]*self.best_coeff))
                logging.info('wd(base,coeff,actual) {:.10e} {:.10e} {:.10e}'.format(wds[0], self.best_wd_coeff, wds[0]*self.best_wd_coeff))

            if not multi_precision:
                if self.zero_bn_decay == 1:
                   for i in range(len(wds)):
                      wds[i] = 0.0
                if self.bn_lr_decay:
                    lrs = [lr * self.best_coeff for lr in lrs]
                ...
                for i, idx in enumerate( indices ):
                    layer_name = self.idx2name[idx]
                    if layer_name.endswith( '_weight' ):
                        lars_flags += ( 1 << i )

                kwargs['best_coeff'] = self.best_coeff 
                kwargs['best_wd_coeff'] = self.best_wd_coeff 

                kwargs['lars_flags'] = lars_flags
                kwargs['world_size'] = 1
                kwargs['eta'] = self.larc_eta

                multi_mp_sgd_two_stage_lars_mom_update(*_flatten_list(zip(weights, grads, tmp_w, *zip(*states))),
                                            out=weights, num_weights=len(weights),
                                            lrs=lrs, wds=wds, **kwargs)
        elif self.larc_scale_active != 0:
            kwargs['world_size'] = 1
            kwargs['eta'] = self.larc_eta
            if not multi_precision:
                if self.zero_bn_decay == 1:
                   for i in range(len(wds)):
                      wds[i] = 0.0
                multi_sgd_larc_mom_update(*_flatten_list(zip(weights, grads, states)), out=weights,
                                         num_weights=len(weights), lrs=lrs, wds=wds, **kwargs)
            else:
                multi_mp_sgd_larc_mom_update(*_flatten_list(zip(weights, grads, *zip(*states))),
                                            out=weights, num_weights=len(weights),
                                            lrs=lrs, wds=wds, **kwargs)
        elif self.larc_clip_active != 0:
            for i in range(len(lrs)):
                if self.print_values and 0 in indices and i == 0:
                    logging.info('lbmult {:.10e}'.format(lbmult))
                    logging.info('lr(base,coeff,actual) {:.10e} {:.10e} {:.10e}'.format(lrs[0], 1.0, lrs[0]))
                    logging.info('wd(base,coeff,actual) {:.10e} {:.10e} {:.10e}'.format(wds[0], self.best_wd_coeff, wds[0]*self.best_wd_coeff))
            ...
            if not multi_precision:
                if self.zero_bn_decay == 1:
                   for i in range(len(wds)):
                      wds[i] = 0.0
                multi_sgd_mom_update_ex(*_flatten_list(zip(weights, grads, states)), out=weights,
                                         num_weights=len(weights), lrs=lrs, wds=wds, **kwargs)
            else:
                multi_mp_sgd_mom_update_ex(*_flatten_list(zip(weights, grads, *zip(*states))),
                                            out=weights, num_weights=len(weights),
                                            lrs=lrs, wds=wds, **kwargs)

    def update(self, index, weight, grad, norms, state):
        self._update_impl(index, weight, grad, norms, state, multi_precision=False)

    def update_multi_precision(self, index, weight, grad, norms, state):
        if not isinstance(index, (tuple, list)):
            use_multi_precision = self.multi_precision and weight.dtype == numpy.float16
        else:
            use_multi_precision = self.multi_precision and weight[0].dtype == numpy.float16
        self._update_impl(index, weight, grad, norms, state,
                          multi_precision=use_multi_precision)
    def _set_best_coeff(self, coeff):
        self.best_coeff = coeff

    def _set_best_wd_coeff(self, coeff):
        self.best_wd_coeff = coeff

    def _set_tmp_coeff(self, coeff):
        self.tmp_coeff = coeff

    def _set_tmp_wd_coeff(self, coeff):
        self.tmp_wd_coeff = coeff

    def _set_cg(self, cg):
        self.clip_gradient = cg
...
class Updater(object):
    ...
    def __call__(self, index, grad, weight):
        ...
        for i, idx in enumerate(indices):
            ...
            if isinstance(idx, bytes):
                ...
                idx = indices[i]
            if idx not in self.tmp_w:
                self.tmp_w[idx] = self.optimizer.create_norms(idx, weights[i])
            ...
            for idx in type_map:
                ...
                while current_index < len(indices):
                    ...
                    for j in range(step):
                        ...
                        norms.append(self.tmp_w[indices[current_index + j]])
    ...
    def states_alloc(self, index, grad, weight):
        """Updates weight given gradient and index."""
        if not isinstance(index, (list, tuple)):
            indices = [index]
            grads = [grad]
