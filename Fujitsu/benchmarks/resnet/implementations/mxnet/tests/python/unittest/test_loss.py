...
def test_poisson_nllloss():
    ...
    brute_loss = np.mean(np.exp(pred.asnumpy()) - target.asnumpy() * pred.asnumpy(), axis=not_axis0)
    loss_withlogits = Loss(pred, target)
    assert_almost_equal(brute_loss, loss_withlogits.asnumpy())
    ...
    np_loss_no_logits = np.mean(pred.asnumpy() - target.asnumpy() * np.log(pred.asnumpy() + 1e-08),
                                axis=not_axis0)
    assert_almost_equal(np_loss_no_logits, loss_no_logits.asnumpy(), equal_nan=True)
    ...
    np_compute_full = np.mean((np_pred - np_target * np.log(np_pred + 1e-08)) +
                              ((np_target * np.log(np_target) - np_target +
                                0.5 * np.log(2 * np_target * np.pi))*(np_target > 1)),
                              axis=not_axis0)
    ...
    assert_almost_equal(np_compute_full, loss_compute_full.asnumpy())
