...
def test_batchnorm_training():
    def check_batchnorm_training(stype):
        for shape in [(2, 3), (3, 2, 4), (2, 3, 2, 2)]:
            ...
            # BatchNorm_v1 testing.  1D input not supported
            if len(shape) != 3:
                test = mx.symbol.BatchNorm_v1(data, fix_gamma=True)
                check_numeric_gradient(test, in_location, mean_std, numeric_eps=1e-2, rtol=rtol, atol=atol)
                ...
                for stride_x in [1, 2]:
                    for pad_x in [0, 1]:
                        for in_size in [7, 32]:
                            ...
                            # test forward
                            for arr1, arr2 in zip(exe1.arg_arrays, exe2.arg_arrays):
                                arr1[:] = np.random.normal(size=arr1.shape).astype(np.float32)
                                arr2[:] = arr1

                            exe1.forward(is_train=True)
                            exe2.forward(is_train=True)

                            # Tolerances are seemingly large because of the innacuracies of the
                            # unfused winograd kernels:
                            # Forward algo 7, backward-to-data algo 5, backward-to-filter algo 5
                            for arr1, arr2 in zip(exe1.outputs, exe2.outputs):
                                assert_almost_equal(arr1.asnumpy(), arr2.asnumpy(), rtol=0.01, atol=0.4)

                            # test backward
                            # use equal dy values to make forward/backward tests independent
                            dy_np = np.random.normal(size=exe1.outputs[0].shape).astype(np.float32)
                            dy1 = mx.nd.array(dy_np, ctx=dev)
                            dy2 = mx.nd.array(dy_np, ctx=mx.cpu())

                            exe1.backward(dy1)
                            exe2.backward(dy2)

                            # dx
                            assert_almost_equal(exe1.grad_arrays[0].asnumpy(),
                                                       exe2.grad_arrays[0].asnumpy(), rtol=0.02, atol=0.8)
                            # dw
                            assert_almost_equal(exe1.grad_arrays[1].asnumpy(),
                                                       exe2.grad_arrays[1].asnumpy(), rtol=0.02, atol=0.8)
                            # db
                            assert_almost_equal(exe1.grad_arrays[2].asnumpy(),
                                                       exe2.grad_arrays[2].asnumpy(), rtol=0.001, atol=0.001)
...
def test_batch_dot():
    ...
    dtypes = ['float32', 'float64']
    if ctx.device_type == 'gpu':
        # Run float16 test only if we have a Maxwell arch GPU or later.
        if get_sm_arch(ctx.device_id) >= 50:
            print('Adding float16 batch gemm test on Maxwell and later.')
            dtypes += ['float16']
        else:
            print('Bypassing float16 batch gemm test on Kepler- no CublasGemmStridedBatchedEx library support.')
...
#     # Make a symbol with 'add' followed by 'relu' Activation, allowing control of:
#     #   1. whether add-relu fusion can take place (by applying a 'probe' to the disappearing node).
#     #   2. the number of uses of the relu output (to test backward gradient-sum fusion with relu).
#     def make_add_relu(kill_fusion, relu_output_uses):
#         a = mx.sym.Variable('a')
#         b = mx.sym.Variable('b')
#         sum = a + b
#         relu = mx.sym.Activation(data=sum, act_type='relu', name='relu')
#         probe = sum if kill_fusion else None
#         if relu_output_uses == 1:
#             return (relu, probe)
#         else:
#             relu_uses = [relu]*relu_output_uses
#             return (mx.sym.add_n(*relu_uses, name='relu'), probe)
#
#     def node_count(sym):
#         return len(sym.get_internals().list_outputs())
#
#     # Test case of add-relu fusion
#     def add_relu_with_options(in_type, in_shape, spray_count):
#         sym_no_fusion, probe = make_add_relu(kill_fusion=True, relu_output_uses=spray_count)
#         sym_fusion, _ = make_add_relu(kill_fusion=False, relu_output_uses=spray_count)
#         sym_list = [sym_no_fusion, sym_fusion]
#         ctx = {'ctx': default_context(), 'a': in_shape, 'b': in_shape, 'type_dict': {'a': in_type, 'b': in_type}}
#         ctx_list = [ctx, ctx]
#         check_consistency(sym_list, ctx_list)
#         assert(node_count(sym_fusion) < node_count(sym_no_fusion))
#
#     for spray_count in [1, 2, 3]:
#         for in_shape in [(16,), (37,), (7, 9), (2,3,4)]:
#             for in_type in [np.float16, np.float32, np.float64]:
#                 add_relu_with_options(in_type, in_shape, spray_count)
#
#     # A previous implementation of add-relu fusion failed to perform the fusion if the same
#     # symbol was bound to a second context and the relu was the symbol's output.
#
#     sym_fusion, _ = make_add_relu(kill_fusion=False, relu_output_uses=1)
#     ctx = default_context()
#     exe0 = sym_fusion.bind(ctx,
#                     {'a': mx.nd.array([1,2],ctx=ctx), 'b': mx.nd.array([-2,3],ctx=ctx)})
#     exe1 = sym_fusion.bind(ctx,
#                     {'a': mx.nd.array([1,2],ctx=ctx), 'b': mx.nd.array([-2,3],ctx=ctx)})
#     exe0_desc_lines = len(exe0.debug_str().split('\n'))
#     exe1_desc_lines = len(exe1.debug_str().split('\n'))
#     if exe0_desc_lines != exe1_desc_lines:
#         print('The following two executor debug strings should have the same structure:')
#         print('**************** exe0 debug str ****************\n {}'.format(exe0.debug_str()))
#         print('**************** exe1 debug str ****************\n {}'.format(exe1.debug_str()))
#     assert(exe0_desc_lines == exe1_desc_lines)
